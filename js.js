// Start of the complete JavaScript game logic
        ...
        function updatePingPong() {
            ...
            // Ball collision with paddles (Player 1 - Top)
            if (pingpong.dy < 0 &&
                pingpong.y - pingpong.radii <= character.y + character.height &&
                pingpong.x >= character.x &&
                pingpong.x <= character.x + character.width
            ) {
                pingpong.dy *= -1; 
            }

            // Ball collision with paddles (Player 2 - Bottom)
            if (pingpong.dy > 0 &&
                pingpong.y + pingpong.radii >= character2.y &&
                pingpong.x >= character2.x &&
                pingpong.x <= character2.x + character2.width
            ) {
                pingpong.dy *= -1; 
            }
